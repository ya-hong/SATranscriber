...
def read(
    ...
    if self.decode_result is None:
        return []
    if not r.is_qulity(self.decode_result):
    ...
    def is_stable(result: DecodingResult):
        return (result.tokens[-1] - stoken) < self.mel_buffer.shape[-1] - r.padding
    ...
    stable_results = [result for result in results if is_stable(result)]
